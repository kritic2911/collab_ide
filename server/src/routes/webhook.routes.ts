...
import { getRoomId } from '../ws/roomManager.js';
...
export const webhookRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/webhooks/github', {
    ...
    if (eventType === 'push') {
      ...
      // Invalidate D2 base cache and publish remote_push via Redis PubSub.
      // This ensures all server instances (horizontal scaling) receive the
      // notification, and the next file open fetches fresh content from GitHub.
      for (const file of changedFiles) {
        const roomId = getRoomId(String(repoId), branch, file);

        // Invalidate the cached base so the next read hits GitHub (L3)
        await baseCache.invalidateBase(roomId);

        // Publish remote_push event via Redis PubSub
        await pubsub.publish(roomId, {
          event: 'base_updated',
          roomId,
          userId: 0, // system event, no user
          payload: {
            type: 'remote_push',
